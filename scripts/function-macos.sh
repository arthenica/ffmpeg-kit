...
display_help() {
  ...
  echo -e "\n'$COMMAND' builds FFmpegKit for macOS platform. By default only one architecture (x86-64) is built \
...
When compilation ends, library bundles are created under the prebuilt folder. By default framework bundles and \
universal fat binaries are created. If --xcframework option is provided then xcframework bundles are created.\n"
...
create_external_library_package() {
  if [[ -n ${FFMPEG_KIT_XCF_BUILD} ]]; then
    ...
    for TARGET_ARCH in "${TARGET_ARCH_LIST[@]}"; do
      local FRAMEWORK_PATH=${BASEDIR}/prebuilt/macos-xcframework/.tmp/macos-${TARGET_ARCH}/$2.framework
      mkdir -p "${FRAMEWORK_PATH}" 1>>"${BASEDIR}"/build.log 2>&1 || exit 1
      local STATIC_LIBRARY_PATH=$(find "${BASEDIR}"/prebuilt/macos-${TARGET_ARCH} -name $3)
    ...
    local XCFRAMEWORK_PATH=${BASEDIR}/prebuilt/macos-xcframework/$2.xcframework
    ...
    local FAT_LIBRARY_PATH=${BASEDIR}/prebuilt/macos-universal/$2-universal
    ...
    for TARGET_ARCH in "${TARGET_ARCH_LIST[@]}"; do
      LIPO_COMMAND+=" $(find "${BASEDIR}"/prebuilt/macos-"${TARGET_ARCH}" -name $3)"
    done
    ...
    if [[ ${RC} -eq 0 ]]; then
      ...
      if [[ ${RC} -eq 0 ]]; then
        ...
        if [[ ${LIBRARY_LIBTHEORA} == "$1" ]]; then
          license_directories=("${BASEDIR}/prebuilt/macos-universal/libtheora-universal" "${BASEDIR}/prebuilt/macos-universal/libtheoraenc-universal" "${BASEDIR}/prebuilt/macos-universal/libtheoradec-universal" "${BASEDIR}/prebuilt/macos-framework/libtheora.framework" "${BASEDIR}/prebuilt/macos-framework/libtheoraenc.framework" "${BASEDIR}/prebuilt/macos-framework/libtheoradec.framework")
        elif [[ ${LIBRARY_LIBVORBIS} == "$1" ]]; then
          license_directories=("${BASEDIR}/prebuilt/macos-universal/libvorbisfile-universal" "${BASEDIR}/prebuilt/macos-universal/libvorbisenc-universal" "${BASEDIR}/prebuilt/macos-universal/libvorbis-universal" "${BASEDIR}/prebuilt/macos-framework/libvorbisfile.framework" "${BASEDIR}/prebuilt/macos-framework/libvorbisenc.framework" "${BASEDIR}/prebuilt/macos-framework/libvorbis.framework")
        elif [[ ${LIBRARY_LIBWEBP} == "$1" ]]; then
          license_directories=("${BASEDIR}/prebuilt/macos-universal/libwebpmux-universal" "${BASEDIR}/prebuilt/macos-universal/libwebpdemux-universal" "${BASEDIR}/prebuilt/macos-universal/libwebp-universal" "${BASEDIR}/prebuilt/macos-framework/libwebpmux.framework" "${BASEDIR}/prebuilt/macos-framework/libwebpdemux.framework" "${BASEDIR}/prebuilt/macos-framework/libwebp.framework")
        elif [[ ${LIBRARY_OPENCOREAMR} == "$1" ]]; then
          license_directories=("${BASEDIR}/prebuilt/macos-universal/libopencore-amrnb-universal" "${BASEDIR}/prebuilt/macos-framework/libopencore-amrnb.framework")
        elif [[ ${LIBRARY_NETTLE} == "$1" ]]; then
          license_directories=("${BASEDIR}/prebuilt/macos-universal/libnettle-universal" "${BASEDIR}/prebuilt/macos-universal/libhogweed-universal" "${BASEDIR}/prebuilt/macos-framework/libnettle.framework" "${BASEDIR}/prebuilt/macos-framework/libhogweed.framework")
        else
          license_directories=("${BASEDIR}/prebuilt/macos-universal/$2-universal" "${BASEDIR}/prebuilt/macos-framework/$2.framework")
        fi
...
get_external_library_version() {
  local library_version=$(grep Version "${BASEDIR}"/prebuilt/macos-"${TARGET_ARCH_LIST[0]}"/pkgconfig/"$1".pc 2>>"${BASEDIR}"/build.log | sed 's/Version://g;s/\ //g')
...
get_target_sdk() {
  echo "$(get_target_arch)-apple-macos${TVOS_MIN_VERSION}"
}
...
get_min_version_cflags() {
  case ${ARCH} in
  arm64 | x86-64)
    echo "-mmacosx-version-min=${MACOS_MIN_VERSION}"
    ;;
  esac
}
...
get_arch_specific_ldflags() {
  ...
  arm64)
    ...
    ;;
  x86-64)
    echo "-arch x86_64 -march=x86-64 -fembed-bitcode -target $(get_target_host)"
    ;;
  esac
}
